...
information in the tag set, even when tagger accuracy goes down as a result of
the increased complexity of the tagging task. Our taggers and parsers are
trained and tested using the annotations of the Norwegian Dependency Treebank.
...
effects of tag set granularity on PoS tagging itself. This includes
investigation of the effects of tag sets on PoS tagging for Swedish
...
of tag set size on tagging was investigated by mapping the original tag
set into smaller subsets designed for parsing. \newcite{Meg:02} argues that a tag set with
complete morphological tags may not be necessary for all NLP applications, for
instance syntactic parsing. The study found that the smallest tag set comprising 26
...
lead to a drop in error rate. \newcite{Meg:02} argues that this shows that the size of the
tag set as well as the type of information in the tags are crucial factors for
tagger performance.  Unfortunately, results for parsing with the various PoS
tag sets are not reported.

Similarly, \newcite{Mac:05} investigated the effects of PoS tag sets on tagger
performance in English, specifically the Wall Street Journal portion of the Penn
Treebank (PTB) \cite{Mar:San:Mar:93}. Based on linguistic
considerations \newcite{Mac:05} mapped the 45 tags of the original PTB tag set
 to more fine-grained tag sets to investigate whether additional linguistic information 
could assist the tagger. Experimenting with both lexically
...
\label{sec:data}
Our experiments our based on the newly developed Norwegian Dependency Treebank (NDT)
...
collaboration with the University of Oslo, and contains manually coded 
syntactic and morphological annotation for 
both varieties of Norwegian; 311 000 tokens of Bokmål and 303 000
tokens of Nynorsk. This paper only report results for Bokmål, the main variant.
The treebanked material mostly comprise newspaper
text, but also include government reports, parliament transcripts and
blog excerpts. The annotation process was supported by
...
inventory as used by the Oslo-Bergen
...
about morphological properties such as definiteness, number and tense. 
Selected subsets of these
features are used in our tag set modifications, where the coarse PoS tag of
relevant tokens is concatenated with one or more features to include 
...
\begin{table}
    ...
    \begin{tabular}{@{}ll@{}}
        ...
        \bottomrule
    \end{tabular}
    \caption{Overview of the original PoS tag set of NDT (excluding punctuation tags).}
    \label{ndttagset}
\end{table}
...
companion website. In our experiments, we train on the training data and
evaluate on the development data, before finally evaluating our optimized tag
set on the held-out test data set.
...
\newcite{Sol:Skj:Ovr:14}, who found the Mate parser \cite{Boh:10} to be the
...
while labeled attachment score (LAS) and unlabeled attachment score (UAS) are
computed by the
...
%as
%we want to investigate how we can improve the linguistic quality of a tag set,
%we will
%only consider tag set modifications we deem linguistically sensible.
%we will not introduce tag set modifications which are not linguistically
...
drastically drops when going from the original to the full tag set. TnT reports
an accuracy of 97.47\% on the original tag set, which is reduced to 93.48\% for
the full tag set. These results confirm our hypothesis that the very high
linguistic quality in the full, fine-grained tag set comes at the expense of
drops in tagger performance.  However, the additional linguistic information
provided by the full tag set improves the parser performance. With the original
tag set, Mate reports an LAS of 87.01\% and a UAS of 90.19\%, which increases
to 87.15\% and 90.39\%, respectively, using the full tag set. As we are looking
for linguistically informed distinctions that improve the syntactic parsing,
these results are promising and serve to indicate that additional linguistic
...
\begin{table}
    ...
    \begin{tabular}{@{}llll@{}}
        \toprule
        \textbf{Tag Set} & \textbf{Acc} &
        \textbf{LAS} & \textbf{UAS} \\
        \midrule
        Original & \textbf{97.47\%} & 87.01\% & 90.19\% \\
        Full & 93.48\% & \textbf{87.15\%} & \textbf{90.39\%} \\
        \bottomrule
    \end{tabular}
...
the observations seen for Swedish in \newcite{Ovr:08}, where finiteness was
...
\begin{table*}[ht]
    ...
    \begin{tabular}{@{}lllll@{}}
        \toprule
        \textbf{Category} & \textbf{Feature(s)} & \textbf{Acc} & \textbf{LAS} &
        \textbf{UAS} \\
        \midrule
        \emph{Baseline} & --- & 97.47\% & 87.01\% & 90.19\% \\
        Noun & Type, case \& definiteness & 97.05\% & 88.81\% & 91.73\% \\
        Verb & Finiteness & 97.35\% & 87.30\% & 90.43\% \\
        Adjective & Degree & 97.41\% & 87.29\% & 90.44\% \\
        Determiner & Definiteness & 97.49\% & 87.30\% & 90.42\% \\
        Pronoun & Type \& case & 97.51\% & 87.30\% & 90.41\% \\
        \bottomrule
    \end{tabular}
...
\begin{table}
    ...
    \begin{tabular}{@{}lllll@{}}
        \toprule
        \textbf{Data} & \textbf{Tag Set} & \textbf{Acc}
        & \textbf{LAS} & \textbf{UAS} \\
        \midrule
        \multirow{2}{*}{Dev}
        & Original & \textbf{97.47\%} & 87.01\% & 90.19\% \\
        %Full & 85.15\% & 93.48\% & 87.15\% & 90.39\% \\
        & Optimized & 96.85\% & \textbf{88.87\%} & \textbf{91.78\%} \\
        \midrule
        \multirow{2}{*}{Test}
        & Original & \textbf{97.30\%} & 86.64\% & 90.07\% \\
        %& Full & 84.27\% & 92.79\% & 87.03\% & 90.16\% \\
        & Optimized & 96.35\% & \textbf{88.55\%} & \textbf{91.41\%} \\
        \bottomrule
    \end{tabular}
