...
in terms of syntactic parsing accuracy. Our results show that the introduction
of morphological distinctions not present in the original tag set, whilst
...
In the current paper, we argue that it is nevertheless possible to
significantly improve the performance of the downstream application of
syntactic parsing by introducing linguistically informed, syntactically
informative distinctions to a tag set. In the following section, we first turn
to the treebank that provides the basis for our experiments and outline its
morphological and syntactic annotation choices.
...
configuration can be seen as a tag set specifically optimized for dependency
...
generalizing syntactic patterns.  This would also increase the
complexity of the tagging task though, which can be expected to lead to a drop in
...
set. These results confirm our hypothesis that the high level of linguistic
information
...
interrogative.  They can furthermore exhibit gender, number and person, while
personal pronouns can be distinguished on case (either accusative or
nominative).
...
from those of the original tag set for statistical significance using Dan
...
\begin{table}
    ...
    \begin{tabular}{@{}ll@{}}
        ...
        \texttt{pron|pers|akk} & Personal pron., accusative \\
        \texttt{pron|pers|nom} & Personal pron., nominative \\
        ...
        \texttt{subst|appell|ub|gen} & Common noun, indef., gen. \\
